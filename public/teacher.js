...
const binKey = `bin_${currentUser?.email}`;
let deletedResources = JSON.parse(localStorage.getItem(binKey)) || [];
...
resourceForm.addEventListener("submit", async (e) => {
  ...
  const thumbURL = document.getElementById("resThumbURL").value.trim();
  const fileURL = "/uploads/" + file.filename;
   


  if (!file || !thumbURL) {
    alert("Please upload a file and provide a thumbnail URL.");
    return;
  }
  ...
  formData.append("title", document.getElementById("resTitle").value.trim());
  formData.append("description", document.getElementById("resDesc").value.trim());
  formData.append("class", document.getElementById("resClass").value);
  formData.append("subject", document.getElementById("resSubject").value.trim());
  formData.append("addedBy", currentUser.email);

  // ⬇️ You can also send thumbURL as a field, OR use a real file upload for thumbnail
  formData.append("thumb", new Blob([])); // Empty blob to avoid backend crash if not using file
  formData.append("thumbnailUrl", thumbURL); // Send thumbnail URL separately (optional)
  ...
  try {
    ...
    if (result.success) {
      ...
      const stored = JSON.parse(localStorage.getItem("global_resources")) || [];
      stored.push(result.resource);
      localStorage.setItem("global_resources", JSON.stringify(stored));
 ...
 document.querySelectorAll(".edit-btn").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    document.getElementById("resLink").value = res.link;
    document.getElementById("resThumb").value = res.thumbnail;
    document.getElementById("resClass").value = res.class;
    document.getElementById("resSubject").value = res.subject;
    
    resourceForm.dataset.editing = id; // store editing mode
    addResourceForm.classList.remove("hidden");
  });
 });
 ...
 filterToggleBtn?.addEventListener("click", () => {
  ...
  const fallbackResources = [
    ...
    },
    {
      ...
      subject: 'History',
      title: 'ppt ',
      description: 'student learning hyb',
      link: 'pdfs/Student_Learning_Hub(ppt).pdf',
      type: 'pdf',
      thumbnail: 'https://imgs.search.brave.com/Bfi8awO6z582VMkPSHeoRB-cF4v5nRKSg1uyC82V92c/rs:fit:860:0:0:0/g:ce/aHR0cHM6Ly9hdXRv/cHB0LmNvbS93cC1j/b250ZW50L3VwbG9h/ZHMvMjAyNC8wOS9U/ZWNobm9sb2d5LXRo/ZW1lLWFpLXBvd2Vy/cG9pbnQtcHJlc2Vu/dGF0aW9uLXRoZW1l/LndlYnA',
      pages:74 ,
      questions: 250
    },
 ...
 // ✅ Step 2: Inject only if localStorage is empty
  ...
  function displayResources(selectedClasses = [], selectedSubjects = []) {
  const resourcesGrid = document.getElementById("resourcesGrid");
  resourcesGrid.innerHTML = "";

  // ✅ Always get updated global + deleted resources
  const globalResources = JSON.parse(localStorage.getItem("global_resources")) || [];
  const deletedResources = JSON.parse(localStorage.getItem(binKey)) || [];

  // ✅ Filter out deleted resources
  const activeResources = globalResources.filter(
    r => !deletedResources.some(d => d.id === r.id)
  );
  ...
  filtered.forEach(resource => {
    ...
    card.innerHTML = `
      <div class="resource-thumbnail" style="background-image: url('${resource.thumbnail}')">
        <span class="resource-type"><i class="fas fa-file"></i> ${resource.type}</span>
      </div>
      <div class="resource-info">
        <h3 class="resource-title">${resource.title}</h3>
        <p class="resource-description">${resource.description}</p>
        <div class="resource-actions">
          <button class="action-btn view-btn" data-id="${resource.id}"><i class="fas fa-eye"></i>👁 View</button>
          ...
          <button class="action-btn delete-btn" data-id="${resource.id}">🗑️ Delete</button>
        </div>
      </div>
    `;
    resourcesGrid.appendChild(card);
  ...
  document.querySelectorAll(".bookmark-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      else bookmarks.push(globalResources.find(r => r.id === id));
...
document.querySelector(".close-viewer")?.addEventListener("click", () => {
  ...
  document.querySelectorAll(".delete-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      const res = globalResources.find(r => r.id === id);
      if (res) {
        const bin = JSON.parse(localStorage.getItem(binKey)) || [];
        bin.push(res);
        localStorage.setItem(binKey, JSON.stringify(bin));
        alert("Moved to bin!");
        displayResources(); // ✅ Refresh view
      }
    });
  });
 }
  ...
  function switchDashboardTab(tabName) {
    ...
    else if (tabName === "bin") renderBinTab();
 ...
 function renderBookmarksTab() {
  ...
  dashboardContent.innerHTML = `<h3>Deleted Resources (Bin)</h3>`;

  if (!deletedResources.length) {
    dashboardContent.innerHTML += `<p>No deleted resources.</p>`;
    return;
  }
  ...
  deletedResources.forEach(resource => {
    dashboardContent.innerHTML += `
      <div class="bin-item">
        ...
        <button class="restore-btn" data-id="${resource.id}">♻️ Restore</button>
        <button class="perma-delete-btn" data-id="${resource.id}">❌ Delete Permanently</button>
        <hr/>
      </div>
    `;
  });
  ...
  btn.addEventListener("click", () => {
    const id = parseInt(btn.dataset.id);
    let deleted = JSON.parse(localStorage.getItem(binKey)) || [];
    deleted = deleted.filter(r => r.id !== id);
    localStorage.setItem(binKey, JSON.stringify(deleted));
  ...
  btn.addEventListener("click", () => {
    ...
    if (confirm("Are you sure you want to permanently delete this resource?")) {
      let deleted = JSON.parse(localStorage.getItem(binKey)) || [];
      let global = JSON.parse(localStorage.getItem("global_resources")) || [];
      deleted = deleted.filter(r => r.id !== id);
      global = global.filter(r => r.id !== id);
      localStorage.setItem(binKey, JSON.stringify(deleted));
      localStorage.setItem("global_resources", JSON.stringify(global));
